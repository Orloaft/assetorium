...
await page.evaluate(([g, c]) => { window.__g = g; window.__c = c; }, [
  ...
  b64("/mnt/nxt-dev/asset-forge/docs/cliff-edge-placeholder.png"),
]);
...
await page.click("button:has-text('New tileset')"); await pickSource(1); await setField("Cols", 16); await setField("Rows", 1); await setField("Cell W", 96); await setField("Cell H", 96); await setField("Output tile size", 96); await page.click("button:has-text('Generate from grid')"); await page.waitForTimeout(400);
...
await page.screenshot({ path: `${SHOT}/33-cliff-edge.png` });
...
await page.screenshot({ path: `${SHOT}/34-cliff-edge-zoom.png` });
