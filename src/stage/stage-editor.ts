...
export function mountStageEditor(root: HTMLElement): Editor {
  ...
  function drawCliffWalls(g: CanvasRenderingContext2D, layer: StageLayer, d: StageDoc, ts: number): void {
    const mem = layer.terrain;
    if (!mem) return;
    for (const t of getProject().terrains) {
      if (!t.wall || !t.wall.tiles.length) continue;
      const h = Math.max(1, t.wall.height);
      const isC = (x: number, y: number): boolean => x >= 0 && y >= 0 && x < d.cols && y < d.rows && mem[y]?.[x] === t.id;
      for (let y = 0; y < d.rows; y++) {
        for (let x = 0; x < d.cols; x++) {
          if (!isC(x, y) || isC(x, y + 1)) continue; // only south-facing edges
          const col = !isC(x - 1, y) ? 0 : !isC(x + 1, y) ? 2 : 1; // L / C / R end of the run
          for (let k = 0; k < h; k++) {
            const wy = y + 1 + k;
            if (wy >= d.rows || isC(x, wy)) break; // off-grid or plateau resumes
            const row = h === 1 ? 0 : k === 0 ? 0 : k === h - 1 ? 2 : 1; // top / mid / base
            const img = L.tileImg.get(t.wall.tiles[row * 3 + col]);
            if (img) g.drawImage(img, 0, 0, img.width, img.height, x * ts, wy * ts, ts, ts);
          }
        }
      }
    }
